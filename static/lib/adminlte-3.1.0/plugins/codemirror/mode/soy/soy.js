    ...
    return {
      ...
      token: function(stream, state) {
        ...
        switch (last(state.soyState)) {
          ...
          case "templ-ref":
            if (match = stream.match(/(\.?[a-zA-Z_][a-zA-Z_0-9]+)+/)) {
              state.soyState.pop();
              // If the first character is '.', it can only be a local templates.
              if (match[0][0] == '.') {
                return "variable-2"
              }
              // Otherwise
          ...
          case "param-type":
            ...
            if ("}]=>,".indexOf(peekChar) != -1) {
              ...
              state.soyState.push('param-type-templates');
              return null;
            } else if (peekChar == "<") {
          ...
          case "tag":
            ...
            if (stream.match(/^\/?}/)) {
              ...
              if (state.tag == "/templates" || state.tag == "/deltemplate") {
          ...
          case "templates-call-expression":
            if (stream.match(/^([\w-?]+)(?==)/)) {
              return "attribute";
            } else if (stream.eat('>')) {
              state.soyState.pop();
        ...
        // Not a tag-keyword; it's an implicit print tag.
          ...
          state.soyState.push("templates-call-expression");
          state.indent += 2 * config.indentUnit;
          state.soyState.push("tag");
          return "keyword";
        } else if (match = stream.match('</>')) {
